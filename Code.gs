...
function doGet(e) {
  ...
  try {
    switch (action) {
      ...
      case 'debugSheets':   result = debugSheets(); break;
      case 'getHeaders':    result = getHeaders(); break;
      case 'checkRows':     result = checkRows(p.sheet||'Lead LG Success', parseInt(p.n||'5',10)); break;
...
function getSheetConfig(name) {
  var cfg = {
    ...
    'Lead LG Success': {
      picCol:9, statusCol:8, notesCol:10,
      parse: function(row) { return {
        name:           clean(row[1]),
        phone:          clean(row[2]),
        email:          '',
...
function getCustomers() {
  ...
  for (var s = 0; s < sheetNames.length; s++) {
    ...
    for (var i = 1; i < data.length; i++) {
      ...
      if (!isPromoter(row[cfg.picCol])) continue;
...
// ── checkRows ───────────────────────────────────────────
function checkRows(sheetName, n) {
  var ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  ...
  for (var i = start; i < data.length; i++) {
    ...
    else if (!isPromoter(picVal))          skip = colLetter(cfg.picCol)+'="'+picVal+'" ไม่ใช่ POND';
...
function isPromoter(v) {
  return normalizeKey(v).indexOf(normalizeKey(PROMOTER)) !== -1;
}
